...
export class VisibleSurfaces {
  ...
  async fillScratchesLength(value: number): Promise<void> {
    await this.expansionPanel
      .getByRole('article') // narrow to article level
      .filter({ hasText: 'Select scratches length (cm)' }) // filter to the scratches article specifically
      .getByRole('slider') // get the slider inside that article
      .fill(String(value));
  }
